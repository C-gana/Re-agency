...
if(!empty($from)&&!empty($to)&&!empty($nop)){
  if($nop>0){
    $ref = rand(5000, 10000);
    $book_sql = "INSERT into bookings(ref_no,unique_id,from_date,to_date,number_of_people,fee,site,add_info)
      values($ref,'$unique_id','$from','$to','$nop','$fee','$dest','$additional_info')";
